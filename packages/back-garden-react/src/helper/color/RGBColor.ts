import { colorNameToHex } from './colorHexHelper';
import { HSLColor } from './HSLColor';

/**
 * description of rgb color
 */
export class RGBColor {
  ...
  public static FromString(color: string) {
    ...
    const normalizedColor = colorNameToHex(color);
  ...
  public static FromHsl(color: HSLColor): RGBColor {
    if (color.saturation === 0) {
      // achromatic
      return new RGBColor(
        Math.round(color.lightness * 255),
        Math.round(color.lightness * 255),
        Math.round(color.lightness * 255),
        color.alpha,
      );
    ...
    return new RGBColor(
      Math.round(finalRed * 255),
      Math.round(finalGreen * 255),
      Math.round(finalBlue * 255),
      color.alpha,
    );
  }
   ...
   */
  reduceHexValue = (value: string): string => {
